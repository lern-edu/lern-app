...
StudentPostCreateForm = React.createClass({
  mixins: [AstroForm(Posts.Schema, 'UserPostSave')],

  componentDidMount() {
    this.defaultHandler({ type: 'info' }, { query: true });
  },
  ...
  */
  ...
  handleTypeChange(event, index, type) {
    this.defaultHandler({ type }, { doc: true, query: true });
  },
  ...
  */
  ...
  render() {
    const { state: { valid, errors }, instructions,
      props: { courses, subjects, tags, images, course, type, documents }, } = this;
    return (
